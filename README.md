...
flowchart LR
  subgraph ENTRY["Entry"]
    Q[User Query] --> META[Metadata<br/>(lang + intent hints)]
  end

  subgraph ROUTE["Viking Routing (scope selection)"]
    META --> VR[Viking Router]
    VR --> VS[Scope<br/>(patterns + conf + trace)]
  end

  subgraph RETR["Retrieval Core"]
    VS --> VEC[Vector search<br/>(scoped)]
    VEC --> FB{soft mode and hits &lt; min_hits<br/>and attempts &lt; max_exp?}

    FB -- yes --> WIDEN[Widen scope<br/>phenom→cat→lang→global]
    WIDEN --> VEC

    FB -- no --> BM25[BM25 keyword search<br/>(global candidates)]
    BM25 --> MODE{Viking mode?}
    MODE -- strict --> BMF[BM25 pre-filter<br/>(in-scope only)]
    MODE -- soft --> SKIP[Skip BM25 pre-filter]

    BMF --> FCHK{hybrid on<br/>and BM25&gt;0?}
    SKIP --> FCHK

    FCHK -- yes --> RRF[RRF fusion<br/>(vector + BM25 candidates)]
    FCHK -- no --> VONLY[Vector-only]
    RRF --> GUARD[Scope guard<br/>strict: hard drop<br/>soft: drop OOS unless in-scope&lt;3]
    VONLY --> GUARD
  end

  subgraph POST["Post-processing (optional)"]
    GUARD --> RAP[Recursive/RAPTOR<br/>(optional)]
    RAP --> RR[Reranker<br/>(optional)]
    RR --> SIB[Sibling expansion<br/>(optional)]
...
flowchart LR
  ...
  CTX --> GD[grade_documents<br/>(optional skip)]
  GD --> DEC{Relevant docs?}
  DEC -- no --> RW[rewrite node]
  RW --> RET

  DEC -- yes --> GEN[generate node]
  GEN --> HC[check_hallucination<br/>(optional skip)]
...
%%{init: {"flowchart": {"curve": "basis", "nodeSpacing": 70, "rankSpacing": 90}}}%%
flowchart LR
  QS[Query Set<br/>(JSONL/YAML)] --> HR[Harness Runner<br/>src/eval/run_retrieval_ablation.py]

  HR --> RET[RAGRetriever.retrieve_documents()<br/>(retriever-core only)]
  RET --> AUD[audit.jsonl<br/>(qid × condition)<br/>docs + scores + source_files<br/>viking_scope + fallback + timing]
  RET --> MET[metrics.json<br/>run meta: git_hash + config_hash + query_hash<br/>per-condition summaries]
  ...
  COND --> HR
  HR --> OUT[Run directory<br/>--out-dir &lt;path&gt;<br/>(one run_id)]
  OUT --> AUD
  OUT --> MET

  NOTE[No UI / No LangGraph imports<br/>(do NOT import src/ui/graph_web.py)]:::warn
  NOTE -.-> HR
